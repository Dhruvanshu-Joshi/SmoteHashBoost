...
def evaluate(
    ...
    # Write to output text log if provided
    if output_file:
        with open(output_file, 'a') as file:
            file.write("\n")
            file.write(f"======[Dataset: {name}]======\n")
    ...
    best_roc_data = None
    best_metrics = [-np.inf] * 5  # [accuracy, AUC, F1, AP, GMean]
    folds = np.zeros((n_runs, 5))

    # Store all results to DataFrame
    excel_rows = []
    ...
    for run in tqdm(range(n_runs)):
        ...
        for fold, (trIndexes, tsIndexes) in enumerate(kFold.split(X, y)):
            ...
            # ✅ Append fold result to Excel data
            excel_rows.append({
                "Dataset": name,
                "Run": run + 1,
                "Fold": fold + 1,
        ...
        end_time = time.time()
        tqdm.write(f"Run {run + 1}/{n_runs} completed in {(end_time - start_time) * 1000:.2f} ms")
    ...
    print()
    print(OUTPUT.format("Best", *np.max(folds, axis=0)))
    print()
    print(OUTPUT.format("Best", *best_metrics))
