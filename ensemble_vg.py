...
class SMOTEHashBasedEnsemble(HashBasedUndersamplingEnsemble):
    def __init__(
            self,
            base_estimator,
            sampling: str = HashBasedUndersamplingEnsemble.RECIPROCAL,
            n_iterations: int = 50,
            random_state: int = None,
            smote_ratio: float = 1.0,  # Ratio of minority class after SMOTE
            k_neighbors: int = 5  # Number of nearest neighbors for SMOTE
    ):
        """
        Combined SMOTE and Hash-Based Undersampling Ensemble for Imbalanced Classification.

        :param base_estimator:
            Base Estimator

        :param sampling: str (default = 'reciprocal')
            Sampling method supported by Hash-Based Undersampling
        ...
        :param smote_ratio: float (default = 1.0)
            Desired ratio of minority samples to majority samples after SMOTE.

        :param k_neighbors: int (default = 5)
            Number of nearest neighbors for SMOTE.
        """
        super().__init__(base_estimator, sampling, n_iterations, random_state)
        self.smote_ratio = smote_ratio
        self.k_neighbors = k_neighbors

    def fit(self, X: np.array, y: np.array):
        """Fitting function that applies SMOTE first, then Hash-Based Undersampling"""

        # Apply SMOTE to generate synthetic samples for the minority class
        smote = SMOTE(sampling_strategy=self.smote_ratio, k_neighbors=self.k_neighbors, random_state=self.random_state)
        X_smote, y_smote = smote.fit_resample(X, y)

        # Use the Hash-Based Undersampling from the parent class
